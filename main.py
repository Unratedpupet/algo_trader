...
historical_data['Signal'][50:] = np.where(historical_data['20_MA'][50:] > historical_data['200_MA'][50:], 1, 0)
...
print("Market: ", historical_data['Cumulative Market Returns'].iloc[-1])
print("Strategy: ", historical_data['Cumulative Strategy Returns'].iloc[-1])

# Calculate and print the Sharpe Ratio
risk_free_rate = 0.01  # This is an example value; you should use a relevant risk-free rate
sharpe_ratio = (historical_data['Strategy Return'].mean() - risk_free_rate) / historical_data['Strategy Return'].std() * np.sqrt(252)
